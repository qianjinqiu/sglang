...
class TestTritonAttention():
    ...
    def _test_extend_attention_once(self, B, N_CTX, H_Q, H_KV, D):
        ...
        b_seq_len_prefix = torch.randint(
            1, N_CTX // 2, (B,), dtype=torch.int32, device="npu"
        )
        ...
        kv_indptr = torch.zeros((B + 1,), dtype=torch.int32, device="npu")
        kv_indptr[1 : B + 1] = torch.cumsum(b_seq_len_prefix[:B], dim=0)
        kv_indices = torch.zeros(
            (b_seq_len_prefix.sum().item(),), dtype=torch.int32, device="npu"
        )
        ...
        k_buffer = torch.empty(
            (total_token_num, H_KV, D), dtype=dtype, device="npu"
        ...
        for i in range(B):
            ...
            q_extend[extend_start:extend_end] = torch.empty(
                (b_seq_len_extend[i], H_Q, D), dtype=dtype, device="npu"
            ).normal_(mean=0.1, std=0.2)

        o_extend = torch.empty((extend_token_num, H_Q, D), dtype=dtype, device="npu")
        o_extend_mask = torch.empty(
            (extend_token_num, H_Q, D), dtype=dtype, device="npu"
        )
        ...
        qo_indptr = torch.zeros((B + 1,), dtype=torch.int32, device="npu")
        ...
        custom_mask = torch.ones(
            (b_seq_mask_len.sum().item(),), dtype=torch.bool, device="npu"
        )
        mask_indptr = torch.zeros((B + 1,), dtype=torch.int64, device="npu")
    ...
    def _test_decode_attention_once(self, B, H_Q, H_KV, D):
        ...
        sm_scale = 1.0 / (D**0.5)
        max_kv_splits = 8
        num_kv_splits = torch.full((B,), 4, dtype=torch.int32, device="npu")
        ...
        kv_indices = torch.arange(total_tokens, device="npu")
    ...
    def _test_grouped_decode_attention_once(self, B, S, H_Q, H_KV, D, D_V):
        ...
        # k_buffer and v_buffer represent all previous tokens
        k_buffer = torch.randn(total_tokens, H_KV, D, dtype=dtype, device="npu")
        v_buffer = torch.randn(total_tokens, H_KV, D_V, dtype=dtype, device="npu")

        # o will have the same shape as q
        o = torch.zeros(B, H_Q, D_V, dtype=dtype, device="npu")
        o_grouped = torch.zeros(B, H_Q, D_V, dtype=dtype, device="npu")
...
if __name__ == "__main__":
    ...
    test.test_extend_attention()
    # test.test_context_attention()
    # test.test_decode_attention()
    # test.test_grouped_decode_attention()
